...
// Override 用于“保留原类型大部分字段，只替换局部字段类型”。
...
>
// Raw 版本允许 number/null/undefined，方便用户输入；
// 进入内部规范化后会被压缩成更稳定的 RouteParamsGeneric。
 ...
 * @internal
 */
// 命名路由对象形态：{ name, params, query, hash, replace, state... }
 ...
 * @internal
 */
// 路径路由对象形态：{ path, query, hash, replace, state... }
...
> {
  // 这是 normalized route 共享的最小骨架，
  // typed-routes 里的各种 RouteLocation* 类型都会基于它再扩展。
  /**
 ...
 */
// 原始 route record 允许直接传组件，也允许传懒加载函数。
...
export interface RouteMeta extends Record<PropertyKey, unknown> {
  // RouteMeta 本身开放扩展，业务可以通过 declare module 'vue-router'
  // 给它增加自定义字段。
  /**
...
export interface RouteRecordSingleView extends _RouteRecordBase {
  // 最普通的单视图路由记录：只有一个 component，没有 children，没有 redirect。
  /**
...
export interface RouteRecordSingleViewWithChildren extends _RouteRecordBase {
  // 允许 children 的单视图记录。component 可以缺省，作为纯父级占位节点存在。
  /**
   * Component to display when the URL matches this route.
   */
...
export interface RouteRecordMultipleViews extends _RouteRecordBase {
  // 命名视图记录：同一路径下可以渲染多个具名 RouterView。
  /**
...
export interface RouteRecordRedirect extends _RouteRecordBase {
  // redirect record 永远不参与真实渲染，只负责把当前命中改写到新的目标地址。
  redirect: RouteRecordRedirectOption
  component?: never
  components?: never
  props?: never
...
export type RouteRecordRaw =
  ...
  | RouteRecordRedirect
// 这是用户写 routes 数组时允许出现的全部 route record 形态联合。
...
// matcher 层只关心 path/name/params 这些匹配信息，不关心 query/hash。

/**
 * Normalized/resolved Route location that returned by the matcher.
 */
export interface MatcherLocation {
  // matcher.resolve() 的产物：已经知道命中谁、params 是什么、matched 链是什么。
  /**
...
// 与 Immutable 相反，用于在内部把只读结构暂时还原成可写视图。
