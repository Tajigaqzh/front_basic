...
function registerGuard(
  ...
  // 组件级守卫最终会被挂到匹配到的 route record 上，
  // 而不是直接挂在组件实例上，方便导航阶段统一抽取执行。
...
export function guardToPromiseFn(
  ...
  // 所有守卫最终都会被适配成 Promise 工厂，router.ts 只需要串行跑队列即可。
  ...
  return () =>
    new Promise((resolve, reject) => {
      const next: NavigationGuardNext = (
        ...
        // next(false) / throw / redirect / next(cb) 都在这里被统一翻译成标准结果。
        if (valid === false) {
          reject(
...
function canOnlyBeCalledOnce(
  ...
  // 旧式 next API 的一个常见坑是重复调用，这里在开发期主动告警。
  let called = 0
  return function () {
    if (called++ === 1)
      warn(
