...
export function createRouter(options: RouterOptions): Router {
  // matcher 负责把 route record 预处理成可匹配、可排序、可 resolve 的内部结构。
  ...
  // pendingLocation 表示“当前这一轮导航真正想去哪里”，用于中途被新导航打断时做取消判定。
  ...
  function pushWithRedirect(
    ...
    // 2. 路由表级别的 redirect 会在真正执行守卫前递归改写目标地址。
    if (shouldRedirect)
      ...
      .then((failure: NavigationFailure | NavigationRedirectError | void) => {
        if (failure) {
          if (
            ...
            // 3. 守卫返回了一个新地址，继续复用当前 push 流程做下一轮导航。
            if (
              __DEV__ &&
          ...
          // 4. 守卫全部通过后，才真正提交当前导航结果并写入 history。
  ...
  function navigate(
    ...
    // 导航守卫执行顺序是 vue-router 的核心语义之一：
    // leave -> global beforeEach -> update -> route beforeEnter
    // -> beforeRouteEnter -> global beforeResolve。
    let guards: Lazy<any>[]
    ...
    return (
      runGuardQueue(guards)
        ...
        .then(() => {
          ...
          // beforeRouteEnter 的回调需要等组件实例创建后再回放，所以这里先清空旧缓存。
  ...
  function finalizeNavigation(
    ...
    // 首次导航和后续 push/replace 的 URL 写入策略不同：
    // 首次导航通常只是在“接管当前地址”，并不会追加一条新 history 记录。
    ...
    // currentRoute 更新后，RouterView / useRoute / 响应式依赖会统一刷新。
  ...
  function setupListeners() {
    ...
    removeHistoryListener = routerHistory.listen((to, _from, info) => {
      ...
      // 浏览器前进/后退会先改 URL，再由 history listener 反向触发一次 router 导航。
