...
function getElementPosition(
  ...
  // 这里计算的是“元素相对文档”的位置，再叠加调用方提供的偏移量。
...
export const computeScrollPosition = (): _ScrollPositionNormalized => ({
  // 统一采集当前窗口滚动位置，history/html5.ts 会在多个时机复用它。
  left: window.scrollX,
  top: window.scrollY,
})

export function scrollToPosition(position: ScrollPosition): void {
  // scrollBehavior 返回的结果既可能是坐标，也可能是元素选择器；
  // 这里负责把两种输入统一翻译成浏览器可执行的 scrollTo 参数。
...
export function getScrollKey(path: string, delta: number): string {
  // 通过 history.state.position + path 组合出一个稳定 key，
  // 从而区分“同一路径但不同历史位置”的滚动状态。
