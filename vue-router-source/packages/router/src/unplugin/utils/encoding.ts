...
// unplugin 侧保留了一份编码实现，原因是文件路由生成阶段也需要对路径/参数做编码，
// 但不希望直接耦合 runtime 路由实现细节。
...
export function commonEncode(text: string | number | null | undefined): string {
  // 与 runtime 版本同理：提供 path/query/hash 公共编码底座。
  return text == null
    ? ''
    : encodeURI('' + text)
        .replace(ENC_PIPE_RE, '|')
...
export function encodeParam(text: string | number | null | undefined): string {
  // param 编码会额外处理 /，防止把单个参数拆成多个 segment。
