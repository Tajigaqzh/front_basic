...
export function definePageTransform({
  ...
  // 这个 transform 处理两种模式：
  // 1. 普通页面模块：移除 definePage() 调用
  // 2. ?definePage 虚拟模块：只保留 definePage 配置对象并导出它
  ...
  if (isExtractingDefinePage) {
    // 这里会提取 definePage 的对象字面量，并保留它依赖到的静态 import。
    ...
    walkAST(routeRecord, {
      // 追踪 definePage 对象真正使用到的标识符，以便过滤无关 import。
      enter(node) {
    ...
    // 普通模式下只需要把 definePage(...) 调用本身从源码里剔掉。
...
export function extractDefinePageInfo(
  ...
  // 这是一个“静态提取优化”：
  // name/path/alias/params 这类简单字段可以直接抽出，不必生成整份 ?definePage 模块。
