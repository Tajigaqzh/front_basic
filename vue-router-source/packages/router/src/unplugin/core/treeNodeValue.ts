...
// TreeNodeValue 是“树节点的路径语义模型”，
// 负责把文件名片段翻译成真正的路由 segment 规则。
...
class _TreeNodeValueBase {
  ...
  constructor(
    ...
    // rawSegment 保留文件系统命名；
    // pathSegment/subSegments 则是转换成路由语义后的结果。
  ...
  get queryParams(): TreeQueryParam[] {
    // query 参数来自 definePage / <route> block 的 params.query 覆盖，而不是文件名本身。
  ...
  get params(): (TreePathParam | TreeQueryParam)[] {
    // 当前节点关心的所有参数 = path params + query params。
  ...
  get overrides() {
    // 多个来源的 override 会按优先级合并：
    // 约定默认值 -> 文件级 override -> build-time edits。
  ...
  setOverride(filePath: string, routeBlock: CustomRouteBlock | undefined) {
    // 不同文件（尤其命名视图）对同一节点的覆盖需要分开存储，再统一合并。
...
export class TreeNodeValueGroup extends _TreeNodeValueBase {
  ...
  readonly score = [300]
  // group 节点通常来自 (group) 这类文件夹命名，用于组织，不一定出现在最终 path 中。
