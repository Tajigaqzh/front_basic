...
export class TreeNode {
  ...
  constructor(
    ...
    // 每个文件路径片段最终都会落成一个 TreeNode，
    // 其 value 则封装“这一段如何转成真正的路由 path 语义”。
  ...
  insertParsedPath(path: string, filePath: string = path): TreeNode {
    // 测试/内部场景下允许直接插入“已解析”的路由 path 片段。
  ...
  setCustomRouteBlock(
    ...
    // <route> block / definePage 的覆盖信息最终都挂在 node.value 的 overrides 上。
  ...
  *getChildrenDeep(): Generator<TreeNode> {
   ...
   */
  get path() {
    // path 是“本节点相对父节点的路由片段”，可能已经被 override 改写。
    return (
