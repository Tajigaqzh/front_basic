...
export function createRoutesContext(options: ResolvedOptions) {
  // 这是文件路由插件的“运行中枢”：
  // 持有 route tree、watchers、param parsers、server context、生成函数等状态。
  ...
  // PrefixTree 保存解析后的页面路由树；
  // EditableTreeNode 是给 extendRoute/beforeWriteFiles 暴露的可编辑包装。
  ...
  async function scanPages(startWatchers = true) {
    // 首次全量扫描：页面文件 + 参数解析器文件，并在结束后生成 routes/dts。
    ...
    await Promise.all([
      ...routesFolder
        ...
        .map(folder => {
          // 每个 routes folder 都会独立建立 watcher 和 glob 扫描任务。
          if (startWatchers) {
  ...
  async function writeRouteInfoToNode(node: TreeNode, filePath: string) {
    // 读取页面源码后，把 definePage 信息和 <route> block 信息写回 tree node。
  ...
  async function addPage(
    ...
    // 新页面文件 -> 插入 route tree -> 回填页面元信息 -> 触发可选扩展逻辑。
  ...
  async function updatePage({ filePath, routePath }: HandlerContext) {
    // 页面更新时不重建整棵树，只更新对应 node 的元信息和可编辑扩展结果。
  ...
  function setupParamParserWatcher(watcher: FSWatcher, cwd: string) {
    // 监听参数解析器目录，变化后重新生成 param parser 类型和运行时代码。
  ...
  function setupWatcher(watcher: RoutesFolderWatcher) {
    // 监听页面目录增删改，并把文件变化映射回 route tree 的增删改。
  ...
  function generateResolver() {
    // 生成 experimental resolver 虚拟模块源码。
    ...
    // ImportsMap 统一收集生成过程中需要的 import，再一次性拼到模块顶部。
