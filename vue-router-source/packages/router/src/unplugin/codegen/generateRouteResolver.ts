 ...
 */
// experimental resolver 复用了与 runtime matcher 接近的排序语义，
// 以保证生成的静态 resolver 命中优先级和真实 router 尽可能一致。
...
// state 用于在递归生成过程中分配唯一变量名，并收集最终要传给 createFixedResolver 的 matchable 记录。
...
export function generateRouteResolver(
  ...
  // 这是 experimental fixed resolver 的总代码生成器：
  // 生成单独的 record 常量声明，再把 matchable records 排序后交给 createFixedResolver。
...
export function generateRouteRecord({
  ...
  // 递归把每个 tree node 生成为一个 normalizeRouteRecord(...) 常量声明，
  // 并在需要时额外生成 alias record。
  ...
  // 纯中间组织节点在 experimental resolver 中可以被折叠掉，减少生成代码量。
  ...
  if (node.needsDefinePageImport) {
    // 与 generateRouteRecords 同理，definePage 的运行时补丁在这里额外 merge。
  ...
  if (!shouldSkipNode) {
    ...
    if (isMatchable) {
      // 只有真正可命中的 record 才会进入 resolver 的匹配列表。
...
`
  ...
  if (varName && isMatchable && aliases && aliases.length > 0) {
    // alias 在 resolver 里会被展开成独立的 normalizeRouteRecord 声明和匹配项。
...
function generateRouteRecordComponent(
  ...
  // experimental resolver 同样支持 named views，只是输出格式面向 normalizeRouteRecord。
 ...
 */
// 下面这组辅助函数负责把 TreeNode 的路径语义翻译成 experimental matcher pattern/runtime 代码。
