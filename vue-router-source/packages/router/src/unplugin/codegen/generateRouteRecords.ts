...
export function generateRouteRecords(
  ...
  // 这是“经典 routes 数组”生成器：
  // 输入 route tree，输出可直接传给 createRouter({ routes }) 的源码字符串。
  if (node.isRoot()) {
    return `[
${node
  .getChildrenSorted()
  ...
  if (node.needsDefinePageImport) {
    // 某些 definePage 字段不能被静态提取，需要额外生成 ?definePage 虚拟导入并在这里 merge。
  ...
  // overrides 是文件约定、<route> block、definePage、build-time edits 合并后的最终结果。
...
${startIndent}}`
  ...
  if (definePageDataList.length > 0) {
    // 有 definePage 运行时数据时，用 experimental 的 _mergeRouteRecord 合并静态 record 和动态补丁。
    // remove one tab
...
function generateRouteRecordComponent(
  ...
  // 单视图时输出 component，多视图时输出 components。
...
export function formatMeta(node: TreeNode, indent: string): string {
  // meta 在 tree 上已经是代码字符串形态，这里只负责带缩进拼回 route record。
