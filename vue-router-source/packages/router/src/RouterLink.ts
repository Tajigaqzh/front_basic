...
export function useLink<Name extends keyof RouteMap = keyof RouteMap>(
  ...
  // useLink 是 RouterLink 的“逻辑核心”，
  // 负责把一个 to props 映射成：resolve 结果、href、高亮状态、点击导航行为。
  ...
  const route = computed(() => {
    ...
    // 每次 to 变化都会重新走 router.resolve，
    // 所以 RouterLink 实际上复用了 matcher/router 的全部解析规则。
    return router.resolve(to)
  })

  const activeRecordIndex = computed<number>(() => {
    // RouterLink 的高亮判断不是简单比较 href，
    // 而是比较 matched record 链和 params，以支持嵌套路由与别名。
  ...
  function navigate(
    ...
    // 只有事件通过 guardEvent 过滤后，才真正触发 router.push/replace。
    if (guardEvent(e)) {
...
export const RouterLinkImpl = /*#__PURE__*/ defineComponent({
  ...
  setup(props, { slots }) {
    // 组件层只负责把 useLink 的结果映射到 class / aria / a 标签渲染。
