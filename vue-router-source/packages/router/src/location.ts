...
export function parseURL(
  ...
  // parseURL 只做轻量字符串拆分，不依赖原生 URL，
  // 这是 vue-router 为了性能和相对路径控制做的取舍。
  // 它负责把原始输入拆成三层：
  // path -> 交给 resolveRelativePath
  // query -> 交给 parseQuery
  // hash -> 交给 decode
...
export function stringifyURL(
  ...
  // stringifyURL 和 parseURL 互为逆过程：
  // path 直接用，query 交给 stringifyQuery，hash 原样拼回。
...
export function stripBase(pathname: string, base: string): string {
  // history 层用它来把浏览器真实地址裁成“router 视角下的相对路径”。
...
export function isSameRouteLocation(
  ...
  // “同一路由”不仅要看 path，还要比较最终命中的 record、params、query、hash。
...
export function resolveRelativePath(to: string, from: string): string {
  // 这里只处理 path 语义，不处理 query/hash；它们会在 parseURL 外层单独拼回去。
