...
export function tokenizePath(path: string): Array<Token[]> {
  // 输出结构是“按 / 切开的 segment 列表”，每个 segment 内再由 static / param token 组成。
  // 例如 `/users/:id(\\d+)?/posts` 会被拆成：
  // [
  //   [{Static: 'users'}],
  //   [{Param: 'id', regexp: '\\d+', optional: true}],
  //   [{Static: 'posts'}]
  // ]
  if (!path) return [[]]
  ...
  function consumeBuffer() {
    ...
    // buffer 代表“当前 token 已经识别完成但还没落盘的字符”。
    buffer = ''
  }
  ...
  while (i < path.length) {
    ...
    switch (state) {
      ...
      case TokenizerState.ParamRegExp:
        // 自定义正则不会做完整的嵌套语法解析，只处理最常见场景；
        // 遇到 \) 会视为正则内容的一部分，而不是结束符。
  ...
  // 返回的是“segment 数组”，不是扁平 token 列表，
  // 因为后续生成 regexp 和 score 时需要 segment 边界信息。
  return tokens
}
