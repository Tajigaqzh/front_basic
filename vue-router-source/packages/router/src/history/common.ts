...
export interface RouterHistory {
  // RouterHistory 是 router 对底层“地址存储/跳转机制”的统一抽象。
  // 只要满足这组接口，router.ts 就不需要关心它是浏览器地址栏、hash，还是内存队列。
  /**
...
export function normalizeBase(base?: string): string {
  // base 会被统一规整为“无尾斜杠”的形式，
  // 后续 createHref / push / replace 才能用简单拼接工作。
  if (!base) {
    if (isBrowser) {
      // respect <base> tag
...
export function createHref(base: string, location: HistoryLocation): string {
  // hash 模式下，base 里可能自带 #，这里会把 # 之前的内容折叠掉，确保 href 格式稳定。
