...
export function buildMatched<T extends EXPERIMENTAL_ResolverRecord_Base>(
  ...
  while (node) {
    // 反向回溯 parent，再 unshift 成 root -> leaf 顺序，符合 route.matched 语义。
  ...
  function validateMatch(record: TRecord, url: LocationNormalized) {
    ...
    const queryParams: MatcherQueryParams = Object.assign(
      ...
      ...matched.flatMap(record =>
        // query matcher 是可叠加的，父子链上所有记录的 query 约束都会一起生效。
  ...
  function resolve(
    ...
    if (typeof to === 'object' && (to.name || to.path == null)) {
      ...
      const query = Object.assign(
        ...
        ...matched.flatMap(record =>
          // 具名导航时 query matcher 会把结构化 params 反推出实际 query 键值。
      ...
      for (record of records) {
        ...
        // root to child) and then match and merge all queries
        try {
          // 顺序扫描记录并尝试命中；第一个完整通过 path/query/hash 校验的记录即成功。
