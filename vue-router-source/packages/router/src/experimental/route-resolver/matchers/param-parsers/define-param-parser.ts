...
export const PARAM_PARSER_DEFAULTS = {
  // query 允许 null 和数组中的 null，因此默认实现尽量保留这些信息。
...
export const PATH_PARAM_PARSER_DEFAULTS = {
  // path 参数不会出现 query 那种数组内 null 的形态，所以这里稍微更严格。
  get: value => value ?? null,
  set: value =>
    value == null
      ? null
...
>(parser: Required<ParamParser<TParam, TUrlParam, TParamRaw>>) {
  // 原样返回，类型系统只负责把调用处声明“冻结”为标准 ParamParser 形状。
  return parser
}
  ...
  return {
    get: value =>
      value == null
        ...
        // repeatable 情况由包装器统一 map，业务 parser 只关注单个字符串。
        : Array.isArray(value)
