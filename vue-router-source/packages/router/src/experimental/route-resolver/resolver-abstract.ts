...
export type MatcherLocationRaw =
  // | `/${string}`
  // 实验版 resolver 接受字符串、具名对象、显式 path 对象、相对对象四大类入口。
  | string
...
export const NO_MATCH_LOCATION = {
  // 未命中时也保持结构稳定，这样调用方仍可安全读取 name/params/matched。
...
export interface ResolverLocationAsNamed {
  name: RecordName
  // 这里故意要求显式给出 params，避免具名跳转时把必填参数遗漏成运行时错误。
  params: MatcherParamsFormatted
...
export interface ResolverLocationAsRelative {
  // 这类对象不重新指定目标记录，只在 currentLocation 基础上局部覆盖字段。
  params?: MatcherParamsFormatted
  query?: LocationQueryRaw
  hash?: string
