...
export function defineBasicLoader<Data>(
  ...
  // defineBasicLoader 是最基础的一种 loader 工厂：
  // 负责把“用户的加载函数 + 一组选项”包装成 useDataLoader 形态。
  ...
  function load(
    ...
    // load 是 loader 内核：确保 entry 存在、处理 pending/staged、运行用户 loader、
    // 根据 lazy/SSR/NavigationResult/error 决定是提交、抛错还是延后。
    ...
    if (!entries.has(loader)) {
      // 首次使用这个 loader 时，为当前 router 建立一份 entry。
      entries.set(loader, {
    ...
    if (entry.pendingTo === to && entry.pendingLoad) {
      // 同一导航目标下的重复触发会直接复用当前 pending promise。
    ...
    const currentLoad = Promise.resolve(
      ...
      .then(d => {
        // loader 成功返回后不会立刻覆盖 data，而是先进入 staged，等待 commit。
        // console.log(
      ...
      .catch((error: unknown) => {
        // 非 lazy 或 SSR 场景下，错误会中断导航；lazy 场景则允许延后暴露。
        if (entry.pendingLoad === currentLoad) {
      ...
      .finally(() => {
        // finally 负责恢复上下文、结束 loading，并在适当时机提交 staged 数据。
  ...
  function commit(
    ...
    // commit 才是“staged -> 正式 data/error”的真正切换点。
