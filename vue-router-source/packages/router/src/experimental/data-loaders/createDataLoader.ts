 ...
 */
// 可以把 entry 理解成“某个 loader 在某个 router 上的运行时状态槽位”：
// 数据、错误、pending、staged、children、commit 行为都集中存这里。
...
> {
  // before/after 是为不同 loader 实现预留的生命周期钩子。
...
// lazy 既可以是布尔值，也可以是基于 to/from 动态决策的函数。
...
export interface UseDataLoader<Data = unknown, TError = unknown> {
  ...
  // loader 本身既是一个 composable（setup 里直接调用拿 refs），
  // 也是一个可 await 的对象（嵌套 loader 里直接 await 拿数据）。
...
export interface UseDataLoaderInternals<Data = unknown, TError = unknown> {
  // 这一组内部方法是导航守卫和嵌套 loader 调度时真正用到的底层接口。
  /**
