...
export function defineColadaLoader<Data>(
  ...
  // 这是基于 Pinia Colada 的 loader 变体。
  // 与 basic loader 相比，它把底层缓存/刷新策略交给了 useQuery/defineQuery。
  ...
  const useDefinedQuery = defineQuery(() => {
    // query 的 key、query 函数、route 跟踪信息都在这里组装。
  ...
  function load(
    ...
    // 逻辑大体和 basic loader 一致，但数据来源变成了 query cache/refetch/refresh。
    const entries = router[LOADER_ENTRIES_KEY]! as _DefineLoaderEntryMap<
      DataLoaderColadaEntry<unknown>
    >
    ...
    if (!entries.has(loader)) {
      // Colada loader 额外维护 tracked/ext/route 这些和 query 缓存有关的运行时状态。
    ...
    if (!entry.ext) {
      // 第一次真正使用时才创建 query 实例，避免提前激活缓存依赖。
    ...
    const currentLoad = ext[reload ? 'refetch' : 'refresh']()
      .then(() => {
        // query 成功后同样先进 staged，仍然复用统一的 commit 语义。
        // console.log(
        //   `✅ resolved ${key}`,
        //   to.fullPath,
        //   `accepted: ${
