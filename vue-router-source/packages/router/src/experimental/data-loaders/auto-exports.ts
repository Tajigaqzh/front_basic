...
export function extractLoadersToExport(
  ...
  // 扫描页面模块的静态 import，把命中 loader 路径白名单的导入名抽出来，
  // 以便后续自动追加到 `export const __loaders = [...]` 中。
...
export function AutoExportLoaders({
  ...
  return {
    ...
    transform: {
      ...
      handler(code) {
        // 找到需要自动导出的 loader 后，直接在模块尾部追加 __loaders 数组导出。
...
export function createAutoExportPlugin(
  ...
  // 再包装成 unplugin 统一接口，供主 unplugin 入口按需挂载。
  return {
    name: PLUGIN_NAME,
