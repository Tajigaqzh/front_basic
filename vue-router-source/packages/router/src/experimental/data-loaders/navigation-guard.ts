...
export function setupLoaderGuard({
  ...
  // 这是 data loaders 与 router 导航生命周期接轨的核心桥梁。
  // beforeEach: 准备本次导航的 loader 容器和 abort signal
  // beforeResolve: 收集并执行 loaders
  // afterEach: commit after-load loaders / 清理 pending
  ...
  const removeLoaderGuard = router.beforeEach(to => {
    // 每次新导航开始时，先终止上一轮未完成的 loader signal。
  ...
  const removeDataLoaderGuard = router.beforeResolve((to, from) => {
    // 此时 matched/components/mods 都已就绪，可以完整扫描当前导航涉及到的全部 loader。
    ...
    return Promise.all(
      loaders.map(loader => {
        // 每个 loader 都运行在 app.runWithContext + effect scope 中，
        // 这样既能 inject/provide，也能和响应式副作用正确绑定。
      ...
      .finally(() => {
        // 守卫结束后要清空 current loader context，避免组件挂载时误以为自己处于 nested loader 环境。
  ...
  const removeAfterEach = router.afterEach((to, from, failure) => {
    // duplicated 等失败不会重新经过 beforeResolve，所以要在 afterEach 里补 pending/reset 处理。
    // console.log(
