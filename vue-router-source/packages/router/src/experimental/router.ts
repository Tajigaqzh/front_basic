...
export interface EXPERIMENTAL_RouteRecordNormalized_Base {
  ...
  // 标准化后统一把 props 规整成按 view name 索引的映射。
  ...
  // 运行时挂载的组件实例，仍然保留给 Options API 的导航守卫使用。
...
export type EXPERIMENTAL_RouteRecordNormalized =
  // 标准化后的总联合，后续 resolver / router 都围绕它运转。
...
export function normalizeRouteRecord(
  ...
  | EXPERIMENTAL_RouteRecordNormalized_Matchable
  | EXPERIMENTAL_RouteRecordNormalized_Group {
  // 先构造一个“可枚举字段完整”的普通对象，再单独把 mods 变成不可枚举。
...
export function mergeRouteRecord(
  ...
  // definePage 等来源只做“浅合并补充”，不覆盖其他运行时字段。
...
export interface EXPERIMENTAL_RouterOptions extends EXPERIMENTAL_RouterOptions_Base {
   ...
   * @experimental
   */
  // 实验版不再直接依赖 matcher，而是依赖一个可插拔 resolver。
...
function mergeMetaFields(
  ...
  // 后面的子记录会覆盖前面父记录同名字段，这与正式版 route.meta 的合并语义一致。
