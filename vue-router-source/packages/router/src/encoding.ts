...
export function commonEncode(text: string | number | null | undefined): string {
  // commonEncode 是最基础的一层，后续 path/query/hash 会在此基础上做差异化补丁。
  // 0 must become '0'
  return text == null
    ? ''
    : encodeURI('' + text)
...
export function encodeHash(text: string): string {
  // hash 的编码规则比 query/path 更宽松，因此会保留一部分字符。
...
export function encodeQueryValue(text: string | number): string {
  // query value 里最容易混淆的是空格和 +：
  // 这里会把空格转成 +，而原始 + 会先编码成 %2B，避免语义冲突。
  return (
    commonEncode(text)
...
export function encodeQueryKey(text: string | number): string {
  // key 比 value 多编码一个 =，否则会破坏 key=value 结构边界。
...
export function encodeParam(text: string | number | null | undefined): string {
  // param 要比 path 更严格，因为它会被塞进单个动态参数槽位中，
  // 斜杠如果不编码，会意外变成新的 segment 分隔符。
...
export function decode(
  ...
  // decode 失败时不抛错，改为保留原值并在开发环境告警，
  // 这样路由系统不会因为某个坏编码直接整体失效。
