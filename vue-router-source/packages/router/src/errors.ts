...
// 这里使用位标记而不是普通枚举值，目的是支持组合判断：
// 例如 aborted | cancelled 可以一次匹配多种失败类型。
...
export function createRouterError<E extends RouterError>(
  ...
  // vue-router 把“导航失败”设计成可识别的 Error 对象，
  // 既保留 throw/catch 兼容性，也允许业务层通过类型位做精确判断。
...
export function isNavigationFailure(
  ...
  // 判断标准不是只看 instanceof Error，
  // 还要看内部 symbol 和 type 位，避免把普通异常误识别成导航失败。
  return (
...
function stringifyRoute(to: RouteLocationRaw): string {
  // 这里只挑 params/query/hash 做日志，是为了把错误信息控制在可读范围内。
