...
// @beginner: 声明 supportsSingletons：保存当前步骤需要读取或更新的数据。
export const supportsSingletons = false;
// @beginner: 声明 resolveSingletonInstance：保存当前步骤需要读取或更新的数据。
export const resolveSingletonInstance = shim;
// @beginner: 声明 acquireSingletonInstance：保存当前步骤需要读取或更新的数据。
export const acquireSingletonInstance = shim;
// @beginner: 声明 releaseSingletonInstance：保存当前步骤需要读取或更新的数据。
export const releaseSingletonInstance = shim;
// @beginner: 声明 isHostSingletonType：保存当前步骤需要读取或更新的数据。
export const isHostSingletonType = shim;
// @beginner: 声明 isSingletonScope：保存当前步骤需要读取或更新的数据。
