...
import { NO, extend } from '@vue-source/shared'
import {
  ErrorCodes,
  createCompilerError,
  defaultOnError,
...
export function baseParse(input: string, options?: ParserOptions): RootNode {
  ...
  if (__DEV__) {
    if (!__BROWSER__ && currentOptions.decodeEntities) {
      console.warn(
        `[@vue-source/compiler-core] decodeEntities option is passed but will be ` +
          `ignored in non-browser builds.`,
      )
    } else if (__BROWSER__ && !__TEST__ && !currentOptions.decodeEntities) {
      throw new Error(
        `[@vue-source/compiler-core] decodeEntities option is required in browser builds.`,
      )
    }
  }
