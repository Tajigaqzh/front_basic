  ...
  walk(root, {
    enter(node: Node & { scopeIds?: Set<string> }, parent: Node | null) {
      ...
      if (node.type === 'Identifier') {
        ...
        if (node.scopeIds) {
          node.scopeIds.forEach((id: string) => markKnownIds(id, knownIds))
        } else {
...
export function extractIdentifiers(
  ...
  switch (param.type) {
    ...
    case 'ArrayPattern':
      param.elements.forEach((element: Node | null) => {
...
function isReferenced(node: Node, parent: Node, grandparent?: Node): boolean {
  switch (parent.type) {
    ...
    case 'ExportSpecifier':
      // eslint-disable-next-line no-restricted-syntax
      if (grandparent && 'source' in grandparent && grandparent.source) {
        return false
      }
