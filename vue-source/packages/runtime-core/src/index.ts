/**
 * 文件作用：runtime-core 对外导出总入口。
 *
 * 这份文件把响应式 API、组件 API、渲染器 API、内置组件和类型统一导出，
 * 是外部使用 runtime-core 时最先进入的模块。
 *
 * 理解这个文件的关键点：
 * - 它几乎不写业务逻辑，主要承担“公共 API 编排层”的角色
 * - 上层包（如 runtime-dom）和最终用户，都会从这里拿到运行时能力
 */

// Core API ------------------------------------------------------------------

// 当前运行时版本号，会在构建阶段由常量替换。
...
// 对外仍保留 `warn` 导出，但生产环境会退化成 NOOP，避免无意义告警代码常驻。
...
// devtools hook 只在开发或 bundler ESM 场景下暴露真实实现，减少生产包体积。
...
declare module '@vue-source/reactivity' {
  export interface RefUnwrapBailTypes {
    runtimeCoreBailTypes:
      | VNode
      | {
          // 不能直接把 ComponentPublicInstance 放进来，否则类型展开会递归；
          // 这里借 `$` 这个内部实例入口告诉 reactivity：“到这里就别继续拆 ref 了”。
          $: ComponentInternalInstance
        }
  }
}

export { TrackOpTypes, TriggerOpTypes } from '@vue-source/reactivity'
export type {
  Ref,
  MaybeRef,
  MaybeRefOrGetter,
