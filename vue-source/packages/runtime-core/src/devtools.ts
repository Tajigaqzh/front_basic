/**
 * 文件作用：实现运行时与 Vue Devtools 的桥接层。
 *
 * 它负责把应用初始化、组件增删改、性能打点、事件派发等运行时信息
 * 转换成 devtools 可识别的 hook 事件。
 *
 * 这份文件不参与渲染决策，
 * 但会贯穿 app 创建、组件挂载/更新/卸载等多个主链路节点做调试上报。
 */
...
// 当 devtools 尚未注入时，先把事件缓存在这里，等 hook 可用后再重放。
let buffer: { event: string; args: any[] }[] = []

let devtoolsNotInstalled = false

/**
 * 作用：向 devtools 发射一条事件；若 hook 尚未就绪，则先放入缓冲区。
 */
...
/**
 * 作用：安装 devtools hook，并把之前缓存的事件批量重放出去。
 *
 * 额外行为：
 * - 浏览器环境下支持“devtools 晚注入”
 * - 若长时间未注入，则清空缓冲避免内存泄漏
 */
