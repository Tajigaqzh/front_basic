/**
 * 文件作用：提供手写 render 时使用的 `h()` API。
 *
 * 这份文件负责把多种重载参数形式统一收口成标准 VNode 创建调用。
 *
 * 和 `createVNode` 的区别：
 * - `createVNode` 更底层、更单态，主要给编译产物使用
 * - `h` 更偏手写 render 友好，允许省略 props、直接传 children 等简写
 */
...
import { type IfAny, isArray, isObject } from '@vue-source/shared'
...
// 这是 `defineComponent` 返回值在 `h()` 重载里的“伪构造器类型”，只用于类型推导。
