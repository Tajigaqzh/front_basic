 ...
 * 文件作用：兼容层能力实现。
 *
 * 当前文件 compatConfig.ts 服务于 compat 模式，
 * 用来兼容 Vue 2 时代的部分运行时行为或 API 语义。
 */

import { extend, hasOwn, isArray, isFunction } from '@vue-source/shared'
import {
  type Component,
...
// 这张表是 compat 模式的核心映射：每一种废弃能力对应一条警告文案和迁移文档链接。
export const deprecationData: Record<DeprecationTypes, DeprecationData> = {
  ...
  [DeprecationTypes.GLOBAL_OBSERVABLE]: {
    ...
    link: `https://vuejs.org/api-source/reactivity-core.html#reactive`,
  },
  ...
  [DeprecationTypes.CONFIG_IGNORED_ELEMENTS]: {
    message: () => {
      ...
      if (isRuntimeOnly()) {
        msg += ` Pass the "isCustomElement" option to @vue-source/compiler-dom instead.`
      } else {
...
// 测试场景下可临时关闭 compat warning，避免断言被噪音干扰。
...
export function warnDeprecation(
  ...
  // 这里统一负责 compat warning 的去重、拼文案、附详情链接以及 disabled 提示。
  if (!__DEV__) {
    return
  }
...
// 全局 compat 配置会和组件级 `compatConfig` 共同决定某项旧行为是否开启。
...
/**
 * 作用：按“组件级优先，全局级兜底”的顺序读取某一项 compat 配置。
 */
...
/**
 * 作用：给“非 compat 构建里已经彻底删除”的旧能力做硬性断言。
 */
...
/**
 * 作用：给“关闭 compat 后很可能直接出错”的旧能力做软断言。
 */
